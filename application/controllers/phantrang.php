...
class Pagination
{
    ...
    {   
        ...
        if ($this->_config['total_record'] > $this->_config['limit'])
        {
            ...
            if ($this->_config['current_page'] > 1)
            {
                $p .= '<a id="'.$this->__link('1').'" href="#'.$this->__link('1').'">First</a>';
                $p .= '<a id="'.$this->__link($this->_config['current_page']-1).'" href="#'.$this->__link($this->_config['current_page']-1).'">Prev</a>';
            }
             */
            ...
            for ($i = 1; $i <= $this->_config['total_page']; $i++)
            {
                ...
                if ($this->_config['current_page'] == $i){
                    $p .= '<a id="'.$i.'" href="'.$this->__link('1').'">'.$i.'</a>';
                }
                else{
            ...
            if ($this->_config['current_page'] < $this->_config['total_page'])
            {
                $p .= '<a id="'.$this->__link($this->_config['current_page'] + 1).'" href="#'.$this->__link($this->_config['current_page'] + 1).'">Next</a>';
                $p .= '<a id="'.$this->__link($this->_config['total_page']).'" href="#'.$this->__link($this->_config['total_page']).'">Last</a>';
            }*/
